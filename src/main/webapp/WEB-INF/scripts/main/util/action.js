(function (window, undefined) {
     ...
     *  @param  {Function} oConf.always 操作的回调
     */
    function fUnFollowTopic(oConf) {
        var that = this;
        that.post({
            url: '/unfollowTopic',
            data: {topicId: oConf.topicId},
            call: oConf.call,
            error: oConf.error,
            always: oConf.always
    });
    ...
    function fPost(oConf) {
        ...
        $.ajax({
            ...
            if (oResult.code === 999) {
                // 未登录
                alert('未登录');
                window.location.href = '/sign_in?next=' + window.encodeURI(window.location.href);
                return;
            }
