(function (window, undefined) {
        ...
        $(document).on('click', '.js-follow-question', function (oEvent) {
            ...
            Action[bFollow ? 'unFollowQuestion' : 'followQuestion']({
                ...
                call: function (oResult) {
                    ...
                    oEl.attr('data-status', bFollow ? '0' : '1');
                    // 按钮颜色
                    oEl.removeClass('zg-btn-white zg-btn-blue').addClass(bFollow ? 'zg-btn-blue' : 'zg-btn-white');
                    // 文字
    ...
    function fFollowTopic() {
        $(document).on('click', '.js-follow-topic', function (oEvent) {
            var oEl = $(oEvent.currentTarget);
            var sId = $.trim(oEl.attr('data-id'));
            if (!sId) {
                return;
            ...
            oEl.attr('data-limit', '1');
            var bFollow = oEl.attr('data-status') === '1';
            Action[bFollow ? 'unFollowTopic' : 'followTopic']({
                topicId: sId,
                call: function (oResult) {
                    // 修改标记位
                    oEl.attr('data-status', bFollow ? '0' : '1');
